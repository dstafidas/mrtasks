function initializeTasks(statusMessages, hiddenStatusMessages, translations, currentPage, currentPageSize) {$(document).ready(function() {
    ...
    $('#editTaskForm').submit(function(event) {
        ...
        if (!formData.get('hoursWorked')) formData.set('hoursWorked', '0');
        if (!formData.get('hourlyRate')) formData.set('hourlyRate', '0');
        if (!formData.get('advancePayment')) formData.set('advancePayment', '0');
    ...
    function initializeBillingFields() {
        ...
        const $billingTypeToggle = $('.billing-type-toggle');
        const $hourlyInputs = $('.hourly-inputs');
        const $fixedInput = $('.fixed-input');
        const $advancePayment = $('#editAdvancePayment').closest('.mb-3');
        const $billingFields = $('#editHoursWorked, #editHourlyRate, #editFixedAmount, #editAdvancePayment');

        function toggleBillingFields() {
            const isChecked = $editBillable.prop('checked');
            $billingTypeToggle.toggle(isChecked);
            $advancePayment.toggle(isChecked);

            const selectedBillingType = $('input[name="billingType"]:checked').val();
            $hourlyInputs.toggle(isChecked && selectedBillingType === 'HOURLY');
            $fixedInput.toggle(isChecked && selectedBillingType === 'FIXED');

            if (!isChecked) {
                $billingFields.val('0').prop('disabled', true);
            } else {
                $('#editAdvancePayment').prop('disabled', false);
                if (selectedBillingType === 'HOURLY') {
                    $('#editHoursWorked, #editHourlyRate').prop('disabled', false);
                    $('#editFixedAmount').prop('disabled', true);
                } else {
                    $('#editHoursWorked, #editHourlyRate').prop('disabled', true);
                    $('#editFixedAmount').prop('disabled', false);
                }
            }
        }

        $editBillable.on('change', toggleBillingFields);
        $('input[name="billingType"]').on('change', toggleBillingFields);
        ...
        $('.edit-task-btn').off('click').on('click', function() {
            ...
            $.getJSON(`/dashboard/task/${taskId}`, function(task) {
                ...
                // Set billing type and values before calling toggleBillingFields
                if (task.billable) {
                    const billingType = task.fixedAmount > 0 ? 'FIXED' : 'HOURLY';
                    $(`input[name="billingType"][value="${billingType}"]`).prop('checked', true);
                    $('#editHoursWorked').val(task.hoursWorked || 0);
                    $('#editHourlyRate').val(task.hourlyRate || 0);
                ...
                // Call toggleBillingFields after setting all values
                toggleBillingFields();
